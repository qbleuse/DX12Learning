...
struct DemoSceneVertex
{
	GPM::vec3 pos;
	GPM::vec2 uv;
};

struct DemoSceneConstantBuffer
{
...
DemoScene::~DemoScene()
{
	...
	if (_skyBoxRootSignature)
		_skyBoxRootSignature->Release();
	if (_skyBoxPso)
		_skyBoxPso->Release();

	if (_cubeMapDescHeap)
		_cubeMapDescHeap->Release();
	if (_cubeMapTexture)
		_cubeMapTexture->Release();

	if (_boxVertexBuffer)
		_boxVertexBuffer->Release();
}

DemoScene::DemoScene(const DemoInputs& inputs_, const DX12Handle& dx12Handle_)
{
	mainCamera.position.z = 1.0f;

	/* make resources */
	if (!MakeModel(dx12Handle_) || !MakeSkyBox(dx12Handle_))
		return;
	...
	for (int i = 0; i < _descHeaps.size(); i++)
	{
		...
		{
			if (!DX12Helper::CreateCBuffer(sizeof(DemoSceneConstantBuffer), _constantBuffers[(i * (_models.size() + 1)) + j], cbUploader))
				break;
		}

		if (!DX12Helper::CreateCBuffer(sizeof(DemoSceneLightBuffer), _constantBuffers[(i * (_models.size() + 1)) + _models.size()], cbUploader))
			break;
	}
	...
	if (!MakeModelShader(vertex, pixel) || !MakeModelPipeline(dx12Handle_, vertex, pixel))
		return;

	if (!MakeSkyBoxShader(vertex, pixel) || !MakeSkyBoxPipeline(dx12Handle_, vertex, pixel))
		return;

	viewport.MaxDepth = 1.0f;
}

bool DemoScene::MakeModelShader(D3D12_SHADER_BYTECODE& vertex, D3D12_SHADER_BYTECODE& pixel)
{
	ID3DBlob* tmp;
...
bool DemoScene::MakeModel(const DX12Handle& dx12Handle_)
{
...
bool DemoScene::MakeModelPipeline(const DX12Handle& dx12Handle_, D3D12_SHADER_BYTECODE& vertex, D3D12_SHADER_BYTECODE& pixel)
{
	HRESULT hr;
	ID3DBlob* error;
	ID3DBlob* tmp;
...
bool DemoScene::MakeSkyBox(const DX12Handle& dx12Handle_)
{
	HRESULT hr;
	...
	int vBufferSize = sizeof(cube);

	D3D12_SUBRESOURCE_DATA vertexData = {};
	vertexData.pData		= (BYTE*)(cube); // pointer to our vertex array
	...
	vertexHelper.buffer = &_boxVertexBuffer;
	...
	hr = dx12Handle_._device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&_cubeMapDescHeap));

	if (FAILED(hr))
	{
		printf("Failing creating skybox descriptor heap for %s: %s\n", Name(), std::system_category().message(hr).c_str());
		return false;
	}

	DX12Helper::TextureResource texture;
	texture.buffer		= &_cubeMapTexture;
	texture.srvHandle	= _cubeMapDescHeap->GetCPUDescriptorHandleForHeapStart();

	if (!DX12Helper::CreateDDSTexture("media/klopenheim_cubemapEnvHDR.dds", texture, uploader))
		return false;
	...
	_boxVBufferView.BufferLocation	= _boxVertexBuffer->GetGPUVirtualAddress();
	_boxVBufferView.StrideInBytes	= sizeof(DemoSceneVertex);
	_boxVBufferView.SizeInBytes		= vBufferSize;

	texture.texData.pData = nullptr;

	return true;
}

bool DemoScene::MakeSkyBoxShader(D3D12_SHADER_BYTECODE& vertex, D3D12_SHADER_BYTECODE& pixel)
{
	ID3DBlob* tmp;
	std::string source = (const char*)R"(#line 518
	struct VOut
	{
		float4 position : SV_POSITION;
		float3 pos : POSITION;
	...
	float4 frag(float4 position : SV_POSITION, float3 pos : POSITION, float2 uv : UV) : SV_TARGET
	{
...
bool DemoScene::MakeSkyBoxPipeline(const DX12Handle& dx12Handle_, D3D12_SHADER_BYTECODE& vertex, D3D12_SHADER_BYTECODE& pixel)
{
	HRESULT hr;
	ID3DBlob* error;
	ID3DBlob* tmp;
	...
	hr = dx12Handle_._device->CreateRootSignature(0, tmp->GetBufferPointer(), tmp->GetBufferSize(), IID_PPV_ARGS(&_skyBoxRootSignature));
	if (FAILED(hr))
	{
		printf("Failing creating root signature of demo %s: %s\n", Name(), std::system_category().message(hr).c_str());
		printf("%s\n", std::system_category().message(dx12Handle_._device->GetDeviceRemovedReason()).c_str());
	...
	{
		{ "POSITION",   0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "UV",			0, DXGI_FORMAT_R32G32_FLOAT,	0, offsetof(DemoSceneVertex,uv), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }
	};
	...
	// create the pso
	hr = dx12Handle_._device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&_skyBoxPso));
	if (FAILED(hr))
	{
		printf("Failing creating skybox PSO of %s: %s\n", Name(), std::system_category().message(hr).c_str());
		printf("%s\n", std::system_category().message(dx12Handle_._device->GetDeviceRemovedReason()).c_str());
		return false;
	}
...
void DemoScene::UpdateAndRender(const DemoInputs& inputs_)
{
	Update(inputs_);
	Render(inputs_);
}

void DemoScene::Update(const DemoInputs& inputs_)
{
	...
	/* random init value */
	static GPM::Vec3 lightDir = { 0.4f, -0.1f , 0.3f };
	static GPM::Vec4 lightColor = { 1.0f,1.0f,0.65f,1.0f };

	DemoSceneLightBuffer lightBuffer = {};
...
void DemoScene::InspectTransform(DX12Helper::Model& model)
{
...
void DemoScene::Render(const DemoInputs& inputs_)
{
	...
	/* update CBuffer */
	DemoSceneConstantBuffer cBuffer = {};
	...
	DrawSkyBox(cmdList, inputs_.renderContext.currFrameIndex);
}

void DemoScene::DrawModel(ID3D12GraphicsCommandList4* cmdList, const DX12Helper::Model& model)
{
...
void DemoScene::DrawSkyBox(ID3D12GraphicsCommandList4* cmdList, int frameIndex)
{
	cmdList->SetGraphicsRootSignature(_skyBoxRootSignature); // set the root signature
	cmdList->SetPipelineState(_skyBoxPso);
	cmdList->RSSetViewports(1, &viewport); // set the viewports
	cmdList->RSSetScissorRects(1, &scissorRect); // set the scissor rects

	cmdList->SetGraphicsRootConstantBufferView(0, _constantBuffers[(frameIndex * (FRAME_BUFFER_COUNT - 1))].buffer->GetGPUVirtualAddress());
	cmdList->SetDescriptorHeaps(1, &_cubeMapDescHeap); // set the descriptor heap
		// set the descriptor table to the descriptor heap (parameter 1, as constant buffer root descriptor is parameter index 0)

	cmdList->SetGraphicsRootDescriptorTable(1, _cubeMapDescHeap->GetGPUDescriptorHandleForHeapStart());

	cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST); // set the primitive topology
	cmdList->IASetVertexBuffers(0, 1, &_boxVBufferView); // set the vertex buffer (using the vertex buffer view)

	cmdList->DrawInstanced(36, 1, 0, 0); // finally draw 36 vertex (draw the cube)
}